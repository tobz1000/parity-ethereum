...
impl ServiceTransactionChecker {
	...
	pub fn check_address<C: CallContract + RegistrarClient>(&self, client: &C, sender: Address) -> Result<bool, String> {
		...
		let contract_address = match client.get_registry_address(
			SERVICE_TRANSACTION_CONTRACT_REGISTRY_NAME,
			BlockId::Latest
		) {
			Ok(Some(contract_address)) => contract_address,
			Ok(None) => return Err("Service transaction contract is not configured".to_owned()),
			Err(e) => return Err(format!("Cannot get service transaction contract address: {}", err)),
		};
	...
	pub fn refresh_cache<C: CallContract + RegistrarClient>(&self, client: &C) -> Result<bool, String> {
		...
		if let Some(contract_address) = client.get_registry_address(SERVICE_TRANSACTION_CONTRACT_REGISTRY_NAME, BlockId::Latest)? {
